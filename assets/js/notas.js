...
export function iniciarRevisionNotasManual({ descargar = true } = {}) {
  ...
  setEstado("Selecciona el reporteglobal.xlsx recién descargado para validar y actualizar las notas.", "warning");
  setProgreso(0);
}
...
export function initNotas() {
  ...
  document.getElementById("reportFileInput")?.addEventListener("change", event => {
    ...
    if (file) setEstado(`${file.name} listo. Presiona “Procesar y guardar notas”.`, "success");
  });
